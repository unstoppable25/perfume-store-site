...
export async function updateProduct(p) {
  ...
  if (kv) {
    try {
      ...
      products[idx] = { ...products[idx], ...p, active: p.active !== false }
      await kv.set('products', products)
      return products[idx]
    } catch (err) {
...
export async function createOrder(order) {
  ...
  if (kv) {
    try {
      ...
      console.log('Order saved to KV successfully')
      return order
    } catch (err) {
  ...
  try {
    ...
    console.log('Order saved to JSON successfully')
  } catch (err) {
...
export async function deleteSubscriber(subscriberId) {
  ...
  if (kv) {
    try {
      ...
      console.error('KV delete subscriber error:', err)
      throw err
    }
  }
  ...
  try {
    fs.writeFileSync(subsFile, JSON.stringify(filteredSubscribers, null, 2))
    return { success: true }
  } catch (err) {
...
export async function deleteMessage(messageId) {
  ...
  if (kv) {
    try {
      ...
      console.error('KV delete message error:', err)
      throw err
    }
  }
...
export async function getSettings() {
  ...
  if (kv) {
    try {
      ...
      console.log('KV getSettings result:', settings)
      if (settings && Object.keys(settings).length > 0) {
        return settings
      }
      // Return empty object if no settings yet
      return {}
    } catch (err) {
      console.error('KV getSettings error:', err)
      throw new Error('Failed to retrieve settings from database: ' + err.message)
    }
  }
  
  // Fallback to JSON (only works locally)
...
export async function updateSettings(key, value) {
  ...
  // Update in KV if available (primary storage for production)
  if (kv) {
    try {
      await kv.hset('site_settings', { [key]: value })
      console.log('Settings updated in KV:', key, 'Type:', Array.isArray(value) ? 'array' : typeof value)
      
      // Get all settings to return
      const allSettings = await kv.hgetall('site_settings')
      return allSettings || { [key]: value }
    } catch (err) {
      console.error('KV updateSettings error:', err)
      throw new Error('Failed to save settings to database: ' + err.message)
    }
  }
  
  // Fallback to JSON (only works locally, not on Vercel)
  try {
    const settingsFile = path.join(DATA_DIR, 'settings.json')
    let settings = {}
    if (fs.existsSync(settingsFile)) {
    ...
    console.log('Settings updated in JSON:', key)
    return settings
  } catch (err) {
    console.error('JSON updateSettings error:', err)
...
export async function storeVerificationCode(email, code, type = 'email_verification') {
  await initKV()
  ...
  const verificationData = {
    ...
    expiresAt: new Date(Date.now() + (type === 'password_reset' ? 15 : 10) * 60 * 1000).toISOString() // 15 min for reset, 10 min for verification
  }
  ...
  if (kv) {
    try {
      ...
      const expirySeconds = type === 'password_reset' ? 900 : 600 // 15 or 10 minutes
      await kv.setex(key, expirySeconds, JSON.stringify(verificationData))
      console.log(`Verification code stored for ${email}, type: ${type}`)
      return true
    } catch (err) {
      console.error('KV storeVerificationCode error:', err)
      throw new Error('Failed to store verification code: ' + err.message)
    }
  }
  ...
  try {
    ...
    if (fs.existsSync(verifyFile)) {
      verifications = JSON.parse(fs.readFileSync(verifyFile, 'utf-8'))
    }
    ...
    console.error('JSON storeVerificationCode error:', err)
    throw new Error('Failed to store verification code: ' + err.message)
  }
}

export async function verifyCode(email, code, type = 'email_verification') {
  await initKV()
  ...
  if (kv) {
    try {
      ...
      if (!data) {
        return { success: false, message: 'Code not found or expired' }
      }
      ...
      console.error('KV verifyCode error:', err)
  ...
  try {
    ...
    const verifications = JSON.parse(fs.readFileSync(verifyFile, 'utf-8'))
    const verificationData = verifications[key]
    ...
    console.error('JSON verifyCode error:', err)
    return { success: false, message: 'Verification failed' }
  }
}
