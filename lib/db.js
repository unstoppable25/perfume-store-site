...
export async function createUser(user) {
  ...
  if (kv) {
    try {
      ...
      console.log('createUser - User data to save:', user.id, 'Length:', userData.length)
      
      await kv.hset('users', { [user.id]: userData })
...
export async function getUserByEmailOrPhone(emailOrPhone) {
  ...
  if (kv) {
    try {
      ...
      if (users) {
        const userArray = Object.values(users).map(u => JSON.parse(u))
        console.log('getUserByEmailOrPhone - Parsed users:', userArray.length)
  ...
  if (kv) {
    try {
      ...
      if (users && Object.keys(users).length > 0) {
        const kvUsers = Object.values(users).map(u => JSON.parse(u))
