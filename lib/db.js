...
export async function updateSettings(key, value) {
  ...
  if (kv) {
    try {
      ...
      console.log('Settings updated in KV:', key, 'Value length:', value?.length)
      
      // Verify the save worked
      const saved = await kv.hget('site_settings', key)
      if (saved !== value) {
        throw new Error('Settings verification failed - saved value does not match')
      }
