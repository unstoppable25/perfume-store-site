...
export async function createUser(user) {
  ...
  if (kv) {
    try {
      await kv.hset('users', { [user.id]: JSON.stringify(user) })
      console.log('User saved to KV:', user.id)
    } catch (err) {
      console.error('KV createUser error:', err)
    }
  }
...
export async function getUserByEmailOrPhone(emailOrPhone) {
  ...
  if (kv) {
    try {
      ...
      console.error('KV getUserByEmailOrPhone error:', err)
    }
  }
