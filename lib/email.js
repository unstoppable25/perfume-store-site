...
export async function sendOrderConfirmationEmail(order) {
  ...
  try {
          ...
          <div style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
              ...
              <tr>
                ...
                <td style="padding: 12px; border-bottom: 1px solid #e5e7eb; text-align: right;">
                  ₦${parseFloat(order.subtotal || order.total || 0).toLocaleString('en-NG')}
                </td>
              </tr>
              ${order.discountAmount && parseFloat(order.discountAmount) > 0 ? `
              <tr>
                <td style="padding: 12px; border-bottom: 1px solid #e5e7eb; color: #059669;">
                  <strong>Discount (${order.discountCode || 'Promo'})</strong>
                </td>
                <td style="padding: 12px; border-bottom: 1px solid #e5e7eb; text-align: right; color: #059669;">
                  -₦${parseFloat(order.discountAmount).toLocaleString('en-NG')}
                </td>
              </tr>
              ` : ''}
              <tr>
    ...
    const result = await client.emails.send({
      from: 'ScentLumus Orders <orders@scentlumus.com>',
